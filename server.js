...
async function processInBackground(projectId, lovableRepoUrl, flutterflowId) {
  ...
  try {
    ...
    const claudePrompt =
      'You are a Flutter Developer. Translate ONLY the Authentication module (src/pages/Auth.tsx, src/hooks/useAuth.tsx) into plain Flutter/Dart based on the SKILL.md rules. Output the generated files directly into a lib/auth/ directory. IGNORE all other screens. Do not ask for confirmation.';
    ...
    await new Promise((resolve, reject) => {
      ...
      childProcess.on('close', async (code) => {
        ...
        try {
          ...
          const dartFiles = entries.filter((file) => String(file).endsWith('.dart'));
